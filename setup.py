...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_pdll", "queries")
...
class EggInfo(egg_info):
    def find_sources(self):
        ...
        self.filelist.recursive_include("src", "*.c")
        self.filelist.recursive_include("src", "*.h")
...
setup(
    ...
    package_data={
        "tree_sitter_pdll": ["*.pyi", "py.typed"],
        "tree_sitter_pdll.queries": ["*.scm"],
    },
    ext_package="tree_sitter_pdll",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_pdll/binding.c",
                "src/parser.c",
            ],
            define_macros=[
